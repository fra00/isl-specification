...
export default function Armory(props: {
    gameSession: {
        ...
        currentMap: {
            ...
            grid: Array<{ x: number; y: number; arnt: { antroc: boolean; inv: boolean; }; mobili: { num: number | null; img: string; }; mostab: { mosid: number; mos: boolean; corpo: number; }; tes: { mon: number; ogg: number | null; arma: number | null; trp: number; }; psgg: { ps: number | null; oriz: boolean; }; trpl: { tipo: number; }; fine: string; }>;
    ...
    onChangePageView: (page: "MAIN_MENU" | "PLAY_GAME" | "EDITOR_GAME" | "SHOP" | "DUNGEON" | "DUNGEON_DESCRIPTION") => void;
}): React.Element;