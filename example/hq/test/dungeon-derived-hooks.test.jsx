...
describe("derived dungeon hooks", () => {
  it("returns visible doors based on opened state and adjacent visible cells", () => {
    ...
    expect(result.current.visibleDoors).toEqual([
      { x: 1, y: 1, img: "portao.png" },
      { x: 3, y: 3, img: "portav.png" },
    ]);
  });
  ...
  it("returns visible furniture and rocks only on non-fogged cells", () => {
          ...
          currentMap: {
            grid: [
              ...
              },
              {
                ...
                mobili: { num: 1, img: "table.png" },
              },
              {
                x: 3,
                y: 3,
                arnt: { antroc: false, inv: false },
                mobili: { num: 1, img: "hidden.png" },
              },
            ],
          },
        },
    ...
    expect(result.current.visibleFurniture).toEqual([
      { x: 1, y: 1, img: "../cell/pietra.png" },
      { x: 2, y: 2, img: "table.png" },
    ]);
  });

  it("filters visible monsters using the board fog map", () => {
    const { result } = renderHook(() =>
      useDungeonVisibleMonsters({
        gameSession: {
          monsters: [{ id: 1, x: 1, y: 1 }, { id: 2, x: 2, y: 2 }, null],
        },
        boardVisibilityMap: {
          data: [
            { x: 1, y: 1, fog: false },
