...
function createSession() {
  return {
    ...
    currentMap: {
      ...
      porte: [{ x: 5, y: 5 }],
      grid: [
        { x: 2, y: 2, tes: { mon: 100, ogg: 0, arma: 0, trp: 0 }, arnt: { antroc: false } },
        { x: 4, y: 4, tes: { mon: 0, ogg: 0, arma: 0, trp: 0 }, arnt: { antroc: false } },
    ...
    heroes: [{
      ...
      turnOrder: 1,
      turnPhase: { HasMoved: true, HasPerformedAction: true, IsTurnFinished: false },
    }, {
      heroId: 2,
      x: 9,
      y: 9,
...
function setup(configOverrides = {}) {
  ...
  const staticEquipment = [{ id: 22, nome: 'Spada Lunga' }, { id: 3, nome: 'Shield', noogg: 2 }];
  const fogOfWarLogic = {
  ...
  const rerender = () => hook.rerender({ gameSession: currentSession, onUpdateSession, onNotify, fogOfWarLogic, staticItems, staticEquipment, ...configOverrides });
  return { ...hook, onNotify, onUpdateSession, fogOfWarLogic, getSession: () => currentSession, rerender };
}
...
function setupDeferred(configOverrides = {}) {
  ...
  return { ...hook, onNotify, onUpdateSession, fogOfWarLogic, getSession: () => currentSession, flushPendingUpdate };
}
...
describe('useDungeonSessionManager', () => {
  it('initializes missions and confirms hero order', () => {
    ...
    expect(api.getSession().heroes[0]).toMatchObject({ x: 1, y: 1, isEscaped: false });
  ...
  it('opens passages, toggles equipment and uses items with effects', () => {
    ...
    api.rerender();
    act(() => {
      expect(api.result.current.toggleEquipItem(1, 3, [{ id: 3, nome: 'Shield', noogg: 2 }])).toBe(true);
    });
    ...
    api.rerender();
    act(() => {
      expect(api.result.current.useItem(1, 7, [{ id: 7, nome: 'Acqua Santa', acqua: true, danni: 2, hp: 1, mp: 1 }], 9)).toBe(true);
    });
  ...
  it('collects treasure, draws cards and applies card effects atomically', () => {
    const api = setup();
    api.getSession().currentMap.grid[0].tes = { mon: 100, ogg: 11, arma: 22, trp: 0 };
    act(() => {
    ...
    expect(api.getSession().heroes[1]).toMatchObject({ gold: 25, inventory: [], equipment: [] });
    expect(api.getSession().currentMap.grid.find((cell) => cell.x === 2 && cell.y === 2).tes.mon).toBe(0);
    expect(api.onNotify).toHaveBeenCalledWith("Hai trovato 100 monete d'oro!\nHai trovato l'oggetto: Pozione del Coraggio!\nHai trovato l'arma: Spada Lunga!");

    api.rerender();
    act(() => {
      expect(api.result.current.drawTreasureCard()).toEqual({ id: 1, azione: 'aggiungi_oro', valore: 50 });
    });
    ...
    act(() => {
      expect(api.result.current.applyTreasureCardEffect(1, { azione: 'mostro_errante' }, onWanderingMonster)).toBe(true);
    });
    expect(onWanderingMonster).toHaveBeenCalledWith(api.getSession().heroes[0].x, api.getSession().heroes[0].y);

    api.rerender();
    act(() => {
      expect(api.result.current.applyTreasureCardEffect(1, { azione: 'aggiungi_oro', valore: 30 }, onWanderingMonster)).toBe(true);
    });
  ...
  it('updates combat, traps, statuses and turn progression', () => {
    ...
    expect(api.getSession().heroes[0]).toMatchObject({ x: 6, y: 6, currentBody: 3 });
    expect(api.getSession().currentMap.grid.find((cell) => cell.x === 4 && cell.y === 4).arnt.antroc).toBe(true);
    ...
    expect(api.getSession().heroes[0]).toMatchObject({ x: 7, y: 7, isEscaped: true });
    expect(api.getSession().heroes[0].turnPhase).toMatchObject({ HasMoved: false, HasPerformedAction: false, IsTurnFinished: true });
