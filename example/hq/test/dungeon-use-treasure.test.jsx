...
describe('useTreasureSearch', () => {
  ...
  it('adds a found treasure only after boundary persistence succeeds', () => {
    useVisibilityCalc.mockReturnValue({ calculateVisibleCells: vi.fn(() => [{ x: 2, y: 2 }]) });
    ...
    const { result } = renderHook(() => useTreasureSearch({
      gameSession: {
        ...
        monsters: [],
        treasureDeck: [],
        currentMap: { grid: [{ x: 2, y: 2, tes: { mon: 100, ogg: 0, arma: 0, trp: 0 } }] },
      },
      visibilityMap: {},
      onNotify,
      onActionDone,
    ...
    expect(result.current.getFoundTreasures()).toEqual([{ x: 2, y: 2, img: 'tesoro.jpg' }]);
  ...
  it('reports persistence failures and falls back to deck drawing only when no static treasure is found', () => {
    ...
    const failed = renderHook(() => useTreasureSearch({
      gameSession: {
        currentTurn: 1,
        heroes: [{ turnOrder: 1, heroId: 4, x: 1, y: 1 }],
        monsters: [],
        treasureDeck: [{ id: 9 }],
        currentMap: { grid: [{ x: 2, y: 2, tes: { mon: 1, ogg: 0, arma: 0, trp: 0 } }] },
      },
      visibilityMap: {},
      onNotify,
      onActionDone: vi.fn(),
      sessionManager: { collectTreasureAtCell: vi.fn(() => false), drawTreasureCard: vi.fn(), applyTreasureCardEffect: vi.fn() },
      onTreasureCardDrawn,
      onWanderingMonster: vi.fn(),
    }));
    ...
    expect(onNotify).toHaveBeenCalledWith('Errore durante la raccolta del tesoro.');
    ...
    const drawn = renderHook(() => useTreasureSearch({
      gameSession: {
        currentTurn: 1,
        heroes: [{ turnOrder: 1, heroId: 4, x: 1, y: 1 }],
        monsters: [],
  ...
  it('blocks search with monsters nearby and delegates treasure card effects', () => {
    ...
    const onNotify = vi.fn();
    const sessionManager = { collectTreasureAtCell: vi.fn(), drawTreasureCard: vi.fn(), applyTreasureCardEffect: vi.fn() };
    ...
    expect(onNotify).toHaveBeenCalledWith('Non puoi cercare tesori con mostri vicini!');
    expect(sessionManager.applyTreasureCardEffect).toHaveBeenCalledWith(4, { id: 9 }, onWanderingMonster);
  });
});