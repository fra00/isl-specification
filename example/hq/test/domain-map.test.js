...
describe('domain-map', () => {
  ...
  it('normalizes primitive map substructures', () => {
    expect(BlockCellArea({ antroc: 1, inv: 0 })).toEqual({ antroc: true, inv: false });
    expect(MapCellFurniture({ num: 2, img: 'chair.png' })).toEqual({ num: 2, img: 'chair.png' });
    expect(MapCellMonster({ mosid: 9, mos: 1, corpo: 3 })).toEqual({ mosid: 9, mos: true, corpo: 3 });
    expect(MapCellTreasure({ mon: 50, ogg: 1, arma: 2, trp: 3 })).toEqual({ mon: 50, ogg: 1, arma: 2, trp: 3 });
    expect(MapCellPassage({ ps: 4, oriz: 1 })).toEqual({ ps: 4, oriz: true });
    expect(MapCellTrap({ tipo: 3, rccadex: 8, rccadey: 9 })).toEqual({ tipo: 3, rccadex: 8, rccadey: 9 });
  });
  ...
  it('builds campaign, mission, visibility and game script records', () => {
    expect(Mission({ ordine: 3, file: 'm.json', titolo: 'Quest' })).toEqual({ ordine: 3, file: 'm.json', titolo: 'Quest' });
    expect(Campaign({ nome_campagna: 'Base', missioni: [{ ordine: 1, titolo: 'One' }] })).toEqual({ nome_campagna: 'Base', missioni: [{ ordine: 1, file: '', titolo: 'One' }] });
    expect(VisibilityCell({ x: 1, y: 2, valo: 'A', vis1: 'B', vis2: 'C', fog: 0 })).toEqual({ x: 1, y: 2, valo: 'A', vis1: 'B', vis2: 'C', fog: false });
    expect(VisibilityMap({ source: 'tbl', image: 'map.png', data: [{ x: 1, y: 2 }] })).toEqual({ source: 'tbl', image: 'map.png', data: [{ x: 1, y: 2, valo: '', vis1: '', vis2: '', fog: true }] });
    expect(GameScript({ command: 'spawn', params: 7, isOneTime: 1 })).toEqual({ command: 'spawn', params: 7, isOneTime: true });
  });
