...
- **Given**: A `MapCell` exists at (5, 5) with `mobili.num` = 101 and `mobili.img` = "chest.jpg". The corresponding `VisibilityCell` at (5, 5) has `fog` = true.
- **When**: `visibleFurniture` is invoked.
- **Assert (Expected Outcomes)**:
...
- **Given**: A `MapCell` exists at (2, 3) with `mobili.num` = 202 and `mobili.img` = "table.jpg". The corresponding `VisibilityCell` at (2, 3) has `fog` = false.
- **When**: `visibleFurniture` is invoked.
- **Assert (Expected Outcomes)**:
...
- **When**: `visibleFurniture` is invoked.
- **Assert (Expected Outcomes)**:
    - The returned list contains `{ x: 10, y: 10, img: "../cell/pietra.jpg" }`.
