...
- **Contract**: Persists a monster position update and/or removes temporary monster statuses inside a single session snapshot.
- **Signature**: `(monsterId: Integer, nextX: Integer | null, nextY: Integer | null, statusesToRemove: List<String>) -> Boolean`
- **Flow**:
  - IF `gameSession` is null RETURN false.
  - Find `monster` in `gameSession.monsters` matching `id` = `monsterId`.
...
- **Contract**: Applies monster combat damage to a hero, clears protective statuses when broken, and persists `lastAttack`.
- **Signature**: `(monsterId: Integer, heroId: Integer, combatResult: @CombatResult) -> Boolean`
- **Flow**:
  - IF `gameSession` is null RETURN false.
  - Find `monster` in `gameSession.monsters` matching `id` = `monsterId`.
  - Find `hero` in `gameSession.heroes` matching `heroId`.
  - IF `monster` is null OR `hero` is null RETURN false.
  - Create `updatedHero` as a copy of `hero`.
  ...
  - Create `updatedSession` as a new @GameSession preserving all unrelated properties.
  - Set `updatedSession.heroes` to `updatedHeroes`.
  - Set `updatedSession.lastAttack` to {hero: `updatedHero`, monster: `monster`, combatResult: `combatResult`}.
  - Trigger `onUpdateSession(updatedSession)`.
  - RETURN true.
...
- **Flow**:
  ...
  - Create `updatedSession` as a new @GameSession preserving all unrelated properties.
  - Set `updatedSession.currentTurn` to 1.
  - Set `updatedSession.heroes` to `updatedHeroes`.
  - Trigger `onUpdateSession(updatedSession)`.
...
- **Flow**:
  - IF `gameSession` is null RETURN false.
  - Find `hero` in `gameSession.heroes` matching `turnOrder` = `gameSession.currentTurn`.
  - IF `hero` is null RETURN false.
  - Create `updatedHero` as a copy of `hero`.
...
- **Flow**:
  ...
  - Find `hero` in `gameSession.heroes` matching `turnOrder` = `gameSession.currentTurn`.
  - Find `monster` in `gameSession.monsters` matching `id` = `monsterId`.
  - IF `hero` is null OR `monster` is null RETURN false.
  - Create `updatedHero` as a copy of `hero`.
  ...
  - Set `updatedSession.heroes` to `updatedHeroes`.
  - Set `updatedSession.monsters` to `updatedMonsters`.
  - Set `updatedSession.lastAttack` to {hero: `updatedHero`, monster: `updatedMonster`, combatResult: `combatResult`}.
  - Trigger `onUpdateSession(updatedSession)`.
  - RETURN true.
...
- **Flow**:
  ...
  - Create `updatedSession` as a new @GameSession preserving all unrelated properties.
  - Set `updatedSession.heroes` to `updatedHeroes`.
  - Set `updatedSession.currentTurn` to `nextTurn`.
  - Trigger `onUpdateSession(updatedSession)`.
  - RETURN true.
