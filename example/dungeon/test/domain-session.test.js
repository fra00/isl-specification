...
describe("domain-session", () => {
  it("creates hero state defaults and copies arrays safely", () => {
    ...
    const hero = HeroState({
      ...
      equipped: [4],
      availableSpells: [5],
      activeStatus: ["Courage"],
      hero: { id: 3, classe: "Elfo" },
    });
  ...
  it("creates a turn phase with false defaults and explicit overrides", () => {
    expect(TurnPhase()).toEqual({
      HasMoved: false,
      HasPerformedAction: false,
      IsTurnFinished: false,
    });
    expect(TurnPhase({ HasMoved: true, IsTurnFinished: true })).toEqual({
      HasMoved: true,
      HasPerformedAction: false,
      IsTurnFinished: true,
    });
