...
- **Assert (Expected Outcomes)**:
    - `gameSession.openedDoors` contains `"10,10"`.
    - `fogOfWarLogic.revealFromPoint(10, 11)` is executed successfully.
    - `onNotify` is called with `"Porta aperta."`.
    - `onUpdateSession` is called with the modified session state.
...
- **Assert (Expected Outcomes)**:
    - The function returns early without modifying `gameSession.openedDoors`.
...
- **When**: `fogOfWarLogic.revealFromPoint` throws an unexpected runtime error.
- **Assert (Expected Outcomes)**:
